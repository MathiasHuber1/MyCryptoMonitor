...
namespace MyCryptoMonitor.Statics
{
    public class MainService
    {
        ...
        public static bool LinesNeedUpdating { get; set; }
        ...
        public static void CheckAlerts(List<CryptocurrencyWithLatestQuote> coins)
        {
            ...
            {
                var coinData = coins.Where(c => c.Symbol.ExtEquals(alert.Coin) && UserConfigService.Currency.ExtEquals(alert.Currency)).FirstOrDefault();

                if (coinData == null)
                    continue;

                var price = coinData.Quote[UserConfigService.Currency].Price;
                if ((alert.Operator == AlertService.Operators.GreaterThan && price > alert.Price && alert.Enabled)
                    || (alert.Operator == AlertService.Operators.LessThan && price < alert.Price && alert.Enabled))
                {
                    triggeredAlerts.Add(alert);
                    alert.Enabled = false;
                }
                else if ((alert.Operator == AlertService.Operators.GreaterThan && price < alert.Price && !alert.Enabled)
                   || (alert.Operator == AlertService.Operators.LessThan && price > alert.Price && !alert.Enabled))
                {
                    alert.Enabled = true;
                }
                else if (alert.Operator == AlertService.Operators.Both && alert.LastOperator == AlertService.Operators.LessThan && price > alert.Price && alert.Enabled)
                {
                    alert.LastOperator = AlertService.Operators.GreaterThan;
                    triggeredAlerts.Add(alert);
                }
                else if (alert.Operator == AlertService.Operators.Both && alert.LastOperator == AlertService.Operators.GreaterThan && price < alert.Price && alert.Enabled)
                {
                    alert.LastOperator = AlertService.Operators.LessThan;
                    triggeredAlerts.Add(alert);
                }
                else if (alert.Operator == AlertService.Operators.Both && alert.LastOperator == null && price > alert.Price && alert.Enabled)
                {
                    alert.LastOperator = AlertService.Operators.GreaterThan;
                }
                else if (alert.Operator == AlertService.Operators.Both && alert.LastOperator == null && price < alert.Price && alert.Enabled)
        ...
        public static void Reset()
        {
            ...
            AlertService.Delete();
            PortfolioService.PortfolioServiceInstance.DeleteAll();
        }
