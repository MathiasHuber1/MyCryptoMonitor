...
namespace CoinMarketCap
{
    ...
    {
        public static event EventHandler<string> ApiError;
        ...
        /// </summary>
        public async Task<Response<List<CryptocurrencyWithLatestQuote>>> GetLatestListingsAsync(ListingLatestParameters request, CancellationToken cancellationToken = default)
        {
        ...
        private async Task<Response<T>> SendApiRequest<T>(object requestParams, string endpoint, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = JsonConvert.DeserializeObject<Response<T>>(content);
                return result;
            }
            catch (Exception ex)
            {
                ApiError?.Invoke("SendApiRequest", ex.Message);
                return null;
            }
        }
